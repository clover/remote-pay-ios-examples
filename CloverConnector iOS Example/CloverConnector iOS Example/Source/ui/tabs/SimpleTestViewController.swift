...
extension PaymentExecutor {
    ...
    fileprivate func processAsSale() {
        ...
        if let txTipModeString = transactionSettings.tipMode?.rawValue,
...
class CapturePreAuthExecutor:BaseExecutor {
    func run() {
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: { [weak self] action in
            guard let self = self,
                let cloverConnector = self.cloverConnector,
                let id = self.payment?.id else { return }
            cloverConnector.addCloverConnectorListener(self)
        ...
        alert.addAction(UIAlertAction(title: "No", style: .cancel, handler: { [weak self] action in
            ...
            VoidPaymentExecutor(cloverConnector: cloverConnector, parentViewController: self.parentViewController, payment: payment).run()
        }))
...
class TipAdjustExecutor:BaseExecutor {
    func run() {
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: { [weak self] action in
            guard let self = self,
                let cloverConnector = self.cloverConnector,
                let orderId = self.payment?.order?.id,
                let id = self.payment?.id else { return }
            cloverConnector.addCloverConnectorListener(self)
        ...
        alert.addAction(UIAlertAction(title: "No", style: .cancel, handler: { [weak self] action in
            guard let self = self,
                let cloverConnector = self.cloverConnector,
                let payment = self.payment else { return }
            RefundPaymentExecutor(cloverConnector: cloverConnector, parentViewController: self.parentViewController, payment: payment).run()
        }))
    ...
    func run() {
        ...
        alert.addAction(UIAlertAction(title: "Partial (Sale only)", style: .default, handler: { [weak self] action in
            guard let self = self,
                ...
                let amount = self.payment?.amount else { return }
            cloverConnector.addCloverConnectorListener(self)
            self.full = false
...
class VoidPaymentExecutor:BaseExecutor, UIAlertViewDelegate {
    ...
    func run() {
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: { [weak self] action in
            ...
            self.cloverConnector?.voidPayment( VoidPaymentRequest(orderId: orderId, paymentId: id, voidReason: .USER_CANCEL))
        }))
        alert.addAction(UIAlertAction(title: "No", style: .cancel, handler: { [weak self] action in
